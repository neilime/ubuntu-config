...
def test_ssh_dir_and_private_keys_exist(host, user_home, is_docker_test_env):
    ...
    # Containers used for testing may set different permissions; only enforce on real systems
    if not is_docker_test_env:
...
def test_gpg_home_and_keys(host, user_home, target_user, is_docker_test_env):
    """User should have a GnuPG home and at least a public key (or list-keys should work)."""
    gnupg = host.file(f"{user_home}/.gnupg")
    if is_docker_test_env and not gnupg.exists:
        pytest.skip("GnuPG not configured in lightweight test container")
