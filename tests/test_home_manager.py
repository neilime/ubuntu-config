...
def test_home_manager_flake_exists(host):
    ...
    assert flake_file.contains("home-manager"), \
...
def test_home_manager_home_nix_exists(host):
    ...
    assert home_file.contains("programs.zsh.enable"), \
        "home.nix should configure zsh"
...
def test_nix_profile_in_shell_config(host):
    ...
    if profile_file.exists:
        assert profile_file.contains("nix-daemon.sh"), \
...
def test_project_template_created(host):
    ...
    if template_dir.exists:
        ...
        assert envrc_template.contains("use flake"), \
            ".envrc should contain 'use flake'"


@pytest.mark.skipif(
    "ansible_module_running_in_container",
    reason="Home Manager requires interactive user session"
)
def test_home_manager_command_available(host):
    """Test that home-manager command is available."""
    user = host.user()
    cmd = host.run_test(
        f"sudo -u {user.name} bash -c '. /nix/var/nix/profiles/default/etc/profile.d/nix-daemon.sh && which home-manager'"
    )
